...
def create_model_object(params: dict) -> list:
    ...
    lgr = LogisticRegression(max_iter=2000000)
...
def get_production_model(list_of_model, training_log, params):
    ...
    if prev_production_model != None:
        ...
        print('<==== prev_production_model["model_data"]["model_object"]', prev_production_model["model_data"])
...
def hyper_params_tuning(model: dict) -> list:
    ...
    # Create model object
    model_gsc = GridSearchCV(model["model_data"]["model_object"], dist_params, n_jobs = -1)
    model_data = {
